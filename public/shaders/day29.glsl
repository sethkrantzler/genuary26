...
void main() {
    ...
    for (int i = 0; i < N*(uPattern+1); i++) {
        ...
        // Mouse influence
        // ------------------------------
        float rotInfluence  = (1./float(uPattern+1))*(uMouse.x - 0.5) * 2.0;   // -1..1 rotation direction & strength
        ...
        if (uPattern == 0) {
            // Filled box
            d = sdBox(q, halfSize);
            line = smoothstep(0.0, -thickness, d);
            ...
            // Filled blobby cross
            d = sdBlobbyCross(q, he);
            line = smoothstep(0.0, -thickness, d);

        } else if (uPattern == 7) {
            // Stroked blobby cross
            d = sdBlobbyCross(q, he);
            line = strokeSDF(d, thickness);
        ...
        // ------------------------------
        vec3 baseColor = vec3(
            0.5 + 0.5 * sin(fi * 1.7*float(uPattern+1) + 0.3*length(uMouse)),
            0.5 + 0.5 * sin(fi * 1.2*float(uPattern+1) + 10.1*uMouse.x),
            0.5 + 0.5 * sin(fi * 2.3*float(uPattern+1) + 20.7*uMouse.y)
        );
